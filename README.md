...
binlog.Log() is 3x faster than fmt.Fprintf() API for an empty string and 30x faster for four arguments. A typical call to *binlog.Log()* is faster than call to *map[string]string* by order of magnitude. The binlog is fast enough to be used in a busy HTTP server.
